...
it.instance("subagent permissions take precedence over parent agent restrictions", () =>
  Effect.gen(function* () {
    ...
    const effective = Permission.merge(permissive.permission, subagentSessionPermission)
...
it.instance("subagent's own read-only restriction remains effective", () =>
  Effect.gen(function* () {
    ...
    const effective = Permission.merge(readonly.permission, subagentSessionPermission)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(Permission.evaluate("edit", "/some/file.ts", parent.permission).action).toBe("deny")
