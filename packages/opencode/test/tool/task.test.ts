  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const build = yield* agent.get("acuflow")
        ...
        const consultant = first.indexOf("- consultant:")
        const zebra = first.indexOf("- zebra: Zebra agent")

        expect(alpha).toBeGreaterThan(-1)
        expect(consultant).toBeGreaterThan(alpha)
        expect(zebra).toBeGreaterThan(consultant)
      }),
    {
      config: {
        agent: {
