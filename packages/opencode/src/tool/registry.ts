...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const episodeStart = yield* EpisodeStartTool
    const patientGet = yield* PatientGetTool
    const patientUpdate = yield* PatientUpdateTool
    const episodeStatus = yield* EpisodeStatusTool
    const clinicalRecord = yield* ClinicalRecordTool
    const clinicalQuery = yield* ClinicalQueryTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          episodeStart: Tool.init(episodeStart),
          patientGet: Tool.init(patientGet),
          patientUpdate: Tool.init(patientUpdate),
          episodeStatus: Tool.init(episodeStatus),
          clinicalRecord: Tool.init(clinicalRecord),
          clinicalQuery: Tool.init(clinicalQuery),
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const visible = (yield* all()).filter((tool) => {
