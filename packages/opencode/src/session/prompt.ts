...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const NODE_TITLES: Record<string, string> = {
      triage: "分诊、首次接诊",
      history_exam: "问诊、查体完成",
      labs: "第一批检验返回",
      imaging_consult: "影像及会诊结果返回",
      treatment_observation: "治疗、留观期间",
      disposition: "住院、转院、出院前",
    }

    // 每轮把当前诊疗节点状态注入系统上下文，确保即使模型未调用 episode_status 也能看到待补充信息。
    const medicalSystemContext = Effect.fn("SessionPrompt.medicalContext")(function* (sessionID: SessionID) {
      const episode = yield* db
        .select()
        .from(EpisodeTable)
        .where(eq(EpisodeTable.session_id, sessionID))
        .get()
        .pipe(Effect.orDie)
      if (!episode?.current_node) return undefined
      const node = yield* db
        .select()
        .from(EpisodeNodeTable)
        .where(and(eq(EpisodeNodeTable.episode_id, episode.id), eq(EpisodeNodeTable.node_key, episode.current_node)))
        .get()
        .pipe(Effect.orDie)
      ...
      return [
        "<acuflow_state>",
        `患者：${patient?.name ?? "匿名"}`,
        `本次诊断：${episode.title || "（未命名）"}`,
        `当前节点：${NODE_TITLES[node.node_key] ?? node.node_key}（${node.node_key}）`,
        `节点状态：${node.status}`,
        ...
        missing.length
          ...
          : "注意：待补充信息为空时，可产出该节点结论草案（fields+summary）并请求医生确认。",
      ].join("\n")
    })
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const acuflow = yield* medicalSystemContext(sessionID)
            const system = [
              ...env,
              ...instructions,
