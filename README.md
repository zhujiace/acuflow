...
### 2. 安装（建议由信息科 / 技术支持一次性完成）

> 临床人员可跳过本节，直接使用信息科配置好的快捷方式。
...
- 或直接解压信息科提供的 `acuflow.zip`（假设解压到 `acuflow` 文件夹）。
...
> （可选）信息科也可以构建独立可执行文件分发给临床：进入 `packages/opencode` 目录后执行 `bun run build --single`，产物位于 `packages/opencode/dist/` 下。
...
| 想让系统直接给出结论 | 流程强制医生确认，这是安全设计；门控细节可由管理员在配置中调整 |
